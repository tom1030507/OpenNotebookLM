...
5. **登入**
   ...
   每次啟動都會確保一組 demo 帳號存在，登入頁也會直接顯示這組帳密並提供
   一鍵填入：
   ...
   可用 `.env` 的 `DEMO_USERNAME` / `DEMO_EMAIL` / `DEMO_PASSWORD` 改成別的。
   帳號已存在時不會被覆蓋，所以改 `DEMO_PASSWORD` 不會重設既有密碼。

   > ⚠️ **對外部署請設 `SEED_DEMO_USER=false`。** 只要它是開的，任何打得開登入頁
   > 的人都看得到一組可用的帳密；把帳號刪掉也沒用，下次啟動會重建。
